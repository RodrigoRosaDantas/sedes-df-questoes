...
const snapshotDir = path.join(root, "data", "notion", "cress-mg-df-20260816");
...
if (releaseDir !== expectedReleaseDir) throw new Error("O lote CRESS MG/DF só pode ser materializado no dist; a release canônica versionada permanece imutável.");
...
const operation = "SEDES-QDX-CRESS-MG-DF-20260816";

const snapshotManifest = readJSON(path.join(snapshotDir, "manifest.json"));
if (snapshotManifest.schema_version !== "1.0" || snapshotManifest.operation_id !== operation) throw new Error("Manifesto imutável CRESS MG/DF ausente ou incompatível.");
if (Number(snapshotManifest.expected_count) !== 12 || Number(snapshotManifest.expected_materials_added) !== 2 || !Array.isArray(snapshotManifest.files) || snapshotManifest.files.length !== 2) {
  throw new Error("Manifesto CRESS MG/DF com escopo inesperado.");
}
...
if (records.length !== 12 || expectedCodes.size !== 12) throw new Error(`Snapshot CRESS MG/DF incompleto: ${records.length}/12.`);
...
const expectedMaterials = Number(source.expected_materials);
const expectedProofs = Number(source.expected_proofs);
...
if (![baselineQuestions, expectedQuestions].includes(startingQuestions)) throw new Error(`Base pública inesperada para o lote CRESS MG/DF: ${startingQuestions}; esperado ${baselineQuestions} ou ${expectedQuestions}.`);
...
function publicQuestion(record) {
  return {
    ...
    auditoria: `Banco Mestre — ${operation} — transcrição, gabarito definitivo e conteúdo auditados`,
...
if (questionById.size !== expectedQuestions) throw new Error(`Lote CRESS MG/DF não fecha o catálogo: ${questionById.size}/${expectedQuestions}.`);
if (startingQuestions === baselineQuestions && added !== 12) throw new Error(`Adição parcial do lote CRESS MG/DF: ${added}/12.`);
...
if (catalog.summary.banco_mestre - catalog.summary.questoes - catalog.summary.discursivas_consulta !== catalog.summary.aguardando_auditoria) throw new Error("Banco Mestre não fecha após o lote CRESS MG/DF.");
...
const receipt = {schema_version: "1.0", operation_id: operation, source_snapshot: "data/notion/cress-mg-df-20260816/manifest.json", status: "success", added_questions: added, total_questions: questionById.size, total_materials: catalogMaterials.length, total_proofs: proofs, banco_mestre: masterTotal, discursivas_consulta: discursive, aguardando_auditoria: expectedAwaiting, codes: [...expectedCodes]};
fs.writeFileSync(path.join(releaseDir, "cress-mg-df-20260816-receipt.json"), `${JSON.stringify(receipt, null, 2)}\n`);
console.log(`✓ Lote CRESS MG/DF materializado no dist: +${added} questões A–E; ${questionById.size} questões, ${catalogMaterials.length} materiais, ${proofs} provas; Banco Mestre ${masterTotal}.`);
