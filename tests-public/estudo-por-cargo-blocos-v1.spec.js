...
test("Estudo por Cargo separa gerais, comuns do nível e específicos do cargo", async ({page}) => {
  ...
  const general202 = page.locator('[data-role-knowledge-group="general"]');
  ...
  await expect(general202).toContainText("Conhecimentos gerais");
  await expect(general202.locator('[data-role-subject="lingua-portuguesa"]')).toBeVisible();
  await expect(general202.locator('[data-role-subject="lei-maria-da-penha"]')).toBeVisible();
  await expect(common202).toContainText("Conhecimentos comuns — nível médio");
  await expect(common202.locator('[data-role-subject="assistencia-social-suas"]')).toBeVisible();
  await expect(common202.locator('[data-role-subject="programas-beneficios-df"]')).toBeVisible();
  ...
  await expect(page.locator("[data-role-level-summary] article")).toHaveCount(3);
  ...
  const general400 = page.locator('[data-role-knowledge-group="general"]');
  ...
  await expect(general400).toContainText("Conhecimentos gerais");
  await expect(general400.locator('[data-role-subject="lingua-portuguesa"]')).toBeVisible();
  await expect(common400).toContainText("Conhecimentos comuns — nível superior");
  await expect(common400.locator('[data-role-subject="assistencia-social-suas"]')).toBeVisible();
  await expect(common400.locator('[data-role-subject="direitos-vulnerabilidades"]')).toBeVisible();
  await expect(common400.locator('[data-role-subject="programas-beneficios-df"]')).toBeVisible();
