...
for (let attempt = 1; attempt <= 30; attempt += 1) {
  try {
    const [buildInfo, catalog, index, app] = await Promise.all([
    ...
    if (buildInfo.builder !== "build-public-v2-11") throw new Error("Pacote público ainda não foi gerado pelo compilador 2.11.");
    ...
    for (const marker of ["app-v4.js?v=6", "reports-v2-10.js?v=2", "manifest.webmanifest"]) {
    ...
    console.log(`✓ Deploy confirmado em ${base}: versão ${buildInfo.version}, commit ${buildInfo.source_sha}, ${questions} questões e ${materials} materiais.`);
    process.exit(0);
  } catch (error) {
    lastError = error;
