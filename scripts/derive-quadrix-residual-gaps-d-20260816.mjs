...
if (manifest.schema_version !== "1.0" || manifest.operation_id !== "SEDES-QDX-RESIDUAL-GAPS-20260816-D") throw new Error("Manifesto residual D inválido.");
if (!Array.isArray(manifest.assignments) || manifest.assignments.length !== 2) throw new Error("Escopo de vínculos do residual D divergente.");
...
const materialsDir = path.join(releaseDir, "materials"), materials = new Map(), questionById = new Map();
...
const publicFormat = internal => internal === "multiple-choice" ? "A–E" : internal === "true-false" ? "Certo/Errado" : "";
...
if (formatSummary["true-false"] !== 2574 || formatSummary["multiple-choice"] !== 937) throw new Error(`Formatos finais inesperados: ${formatSummary["true-false"]} C/E + ${formatSummary["multiple-choice"]} A–E.`);
...
const editalMap = readJSON(editalMapPath), editalItems = new Map((editalMap.sections || []).flatMap(s => (s.items || []).map(i => [i.id, i]))), editalFormats = {...(editalMap.question_formats || {})}, mappedBefore = new Set([...editalItems.values()].flatMap(i => i.question_ids || []));
const preExistingAssignments = manifest.assignments.filter(a => editalItems.get(a.item_id)?.question_ids?.includes(a.question_id)).length;
if (![0, manifest.assignments.length].includes(preExistingAssignments)) throw new Error(`Aplicação parcial do residual D detectada: ${preExistingAssignments}/${manifest.assignments.length}.`);
let newlyMapped = 0;
for (const assignment of manifest.assignments) {
  const record = questionById.get(key(assignment.question_id));
  if (!record || !catalog.question_index?.[assignment.question_id]) throw new Error(`Questão residual D ausente: ${assignment.question_id}.`);
  ...
  if (clean(record.q.gabarito) !== clean(assignment.answer)) throw new Error(`${assignment.question_id}: gabarito diverge do manifesto.`);
  if (sha256(clean(record.q.enunciado)) !== assignment.prompt_sha256) throw new Error(`${assignment.question_id}: fingerprint do enunciado diverge do manifesto.`);
  const detected = publicFormat(formats[record.q.id]); if (detected !== assignment.format) throw new Error(`${assignment.question_id}: formato ${detected} diverge de ${assignment.format}.`);
  const item = editalItems.get(assignment.item_id); if (!item) throw new Error(`Item do edital inexistente: ${assignment.item_id}.`);
  if (!mappedBefore.has(assignment.question_id)) newlyMapped += 1;
  item.question_ids = unique([...(item.question_ids || []), assignment.question_id]); item.exam_question_ids = unique([...(item.exam_question_ids || []), assignment.question_id]);
  if (assignment.format === "A–E") item.ae_question_ids = unique([...(item.ae_question_ids || []), assignment.question_id]);
  else if (assignment.format === "Certo/Errado") item.ce_question_ids = unique([...(item.ce_question_ids || []), assignment.question_id]);
  else throw new Error(`${assignment.question_id}: formato de mapa não suportado.`);
  editalFormats[assignment.question_id] = assignment.format;
}
const expectedNewlyMapped = Number(manifest.expected_newly_mapped_distinct_questions);
if (preExistingAssignments === 0 && newlyMapped !== expectedNewlyMapped) throw new Error(`Questões recém-mapeadas divergentes: ${newlyMapped}.`);
if (preExistingAssignments === manifest.assignments.length && newlyMapped !== 0) throw new Error(`Reaplicação residual D inconsistente: ${newlyMapped} questões reapareceram como novas.`);
...
for (const [itemId, minimum] of Object.entries(manifest.expected_minimums || {})) { const count = Number(editalItems.get(itemId)?.question_count || 0); if (count < Number(minimum)) throw new Error(`${itemId}: cobertura ${count}, mínimo ${minimum}.`); }
for (const [itemId, exact] of Object.entries(manifest.protected_exact || {})) { const count = Number(editalItems.get(itemId)?.question_count || 0); if (count !== Number(exact)) throw new Error(`${itemId}: cobertura protegida mudou de ${exact} para ${count}.`); }
const mappedIds = new Set([...editalItems.values()].flatMap(i => i.question_ids || []));
if (mappedIds.size !== Number(manifest.expected_mapped_questions) || expectedQuestions - mappedIds.size !== Number(manifest.expected_unmapped_questions)) throw new Error(`Mapa final inesperado: ${mappedIds.size} mapeadas / ${expectedQuestions - mappedIds.size} não mapeadas.`);
editalMap.generated_at = manifest.captured_at;
editalMap.summary = {...(editalMap.summary || {}), catalog_questions: expectedQuestions, catalog_multiple_choice_ae: 937, catalog_true_false: 2574, catalog_exam_eligible: expectedQuestions, mapped_questions: mappedIds.size, unmapped_questions: expectedQuestions - mappedIds.size};
fs.writeFileSync(editalMapPath, `${JSON.stringify(editalMap, null, 2)}\n`);

const receipt = {schema_version: "1.0", operation_id: manifest.operation_id, status: "success", catalog_additions: 2, mapping_pairs: manifest.assignments.length, distinct_questions: new Set(manifest.assignments.map(item => item.question_id)).size, newly_mapped_distinct_questions: expectedNewlyMapped, mapped_questions: mappedIds.size, unmapped_questions: expectedQuestions - mappedIds.size, expected_minimums: manifest.expected_minimums, protected_exact: manifest.protected_exact, codes: manifest.expected_codes};
fs.writeFileSync(path.join(releaseDir, "quadrix-residual-gaps-d-20260816-map-receipt.json"), `${JSON.stringify(receipt, null, 2)}\n`);
console.log(`✓ Derivação residual D: +2 C/E e ${receipt.mapping_pairs} vínculos; ${expectedQuestions} questões; ${mappedIds.size} mapeadas; gestão da mudança>=5; proteções preservadas.`);
