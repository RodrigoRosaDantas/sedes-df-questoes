(() => {
  ...
  const remainingLabel = (countdown) => countdown.finished
    ? "Data da prova alcançada"
    : `${countdown.days} dias e ${String(countdown.hours).padStart(2, "0")} horas até a prova`;

  const renderRoleChips = () => (exam.cargos || []).map(role => `
    <span class="exam-role-chip" title="Nível ${escapeHTML(role.nivel)}">
      <b>${escapeHTML(role.carreira)} ${escapeHTML(role.codigo)}</b> · ${escapeHTML(role.nome)}
    </span>`).join("");

  const renderExamCard = () => {
    const countdown = getCountdown();
    const questions = catalog?.summary?.questoes || 0;
    const remainingDays = Math.max(1, countdown.days || 1);
    const dailyPace = countdown.finished ? 0 : Math.max(1, Math.ceil(questions / remainingDays));
    ...
    const levels = (exam.niveis || []).join(" e ");
    const roleCount = (exam.cargos || []).length;
    ...
    return `<section id="exam-countdown" class="exam-focus card" aria-labelledby="exam-title">
      <div class="exam-main">
        <div class="exam-heading">
          ...
          <div class="exam-copy">
            ...
            <h2 id="exam-title">${escapeHTML(exam.titulo_painel)}</h2>
            <p>${escapeHTML(exam.descricao_painel)} ${escapeHTML(exam.etapa)} · ${escapeHTML(exam.banca)}.</p>
            <div class="exam-meta-line"><span>${escapeHTML(weekday)}</span><span>${escapeHTML(numericDate)}</span><span>Brasília/DF</span><span>Níveis ${escapeHTML(levels)}</span></div>
          </div>
        </div>
        <div class="exam-role-list" aria-label="Cargos acompanhados">${renderRoleChips()}</div>
      ...
      <aside class="exam-side-info" aria-label="Informações da prova">
        ...
        <div class="exam-fact"><span>Perfis acompanhados</span><strong>${escapeHTML(levels)} · ${roleCount} cargos</strong></div>
  ...
  const renderPreparationInsight = () => {
    ...
    return `<section id="preparation-insight" class="preparation-insight card">
      <div><i>↗</i><span><strong>${remainingLabel(countdown)}</strong><small>${message}</small></span></div>
  ...
  const updateCountdown = () => {
    ...
    document.querySelector("#preparation-insight strong")?.replaceChildren(remainingLabel(countdown));
