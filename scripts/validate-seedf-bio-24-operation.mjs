...
const marker = JSON.parse(fs.readFileSync(path.join(root, 'data/operations/prepare-seedf-bio-24.json'), 'utf8'));
...
if (marker.scope?.expected_count !== 24
  || marker.scope?.first_original_number !== 97
  || marker.scope?.last_original_number !== 120
  || marker.scope?.code_prefix !== prefix) {
  fail('Escopo registrado não corresponde às questões 97 a 120 de Biologia.');
}

const candidates = (snapshot.records || []).filter(record => !clean(record.github_id));
if (candidates.length !== 24) {
  fail(`A operação exige exatamente 24 registros sem Código GitHub; encontrados ${candidates.length}.`);
}
...
for (const record of candidates) {
  ...
  if (Number(record.original_number) < 97 || Number(record.original_number) > 120) {
...
if (plan.total_records !== 24 || !Array.isArray(plan.lots) || plan.lots.length !== 1) {
  fail('Plano de publicação deve conter exatamente um lote e 24 registros.');
}
const [lot] = plan.lots;
if (lot.lot !== operationId || lot.expected_count !== 24 || JSON.stringify(lot.codes) !== JSON.stringify(expectedCodes)) {
  fail('Plano de publicação não corresponde exatamente ao lote autorizado de Biologia.');
}
...
console.log('✓ Operação restrita validada: 24 questões de Biologia (97–120), um único lote e acervo anterior preservado.');
