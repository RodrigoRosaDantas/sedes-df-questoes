...
const snapshotDir = path.join(root, "data", "notion", "quadrix-residual-gaps-d-20260816");
...
if (releaseDir !== expectedReleaseDir) throw new Error("O lote residual D só pode ser materializado no dist; data/release permanece imutável.");
...
const operation = "SEDES-QDX-RESIDUAL-GAPS-20260816-D";

const snapshotManifest = readJSON(path.join(snapshotDir, "manifest.json"));
if (snapshotManifest.schema_version !== "1.0" || snapshotManifest.operation_id !== operation) throw new Error("Manifesto residual D ausente ou incompatível.");
if (Number(snapshotManifest.expected_count) !== 2 || Number(snapshotManifest.expected_materials_added) !== 0 || !Array.isArray(snapshotManifest.files) || snapshotManifest.files.length !== 1) throw new Error("Manifesto residual D com escopo inesperado.");
...
if (records.length !== 2 || expectedCodes.size !== 2) throw new Error(`Snapshot residual D incompleto: ${records.length}/2.`);
...
for (const record of records) {
  ...
  if (record.material_type !== "Prova" || record.format !== "Certo / Errado" || !new Set(["Certo", "Errado"]).has(String(record.answer || ""))) throw new Error(`${record.code}: tipo, formato ou gabarito inválido.`);
...
if (materialNames.size !== 1) throw new Error(`O lote deve reutilizar exatamente 1 material; encontrados ${materialNames.size}.`);

const catalogPath = path.join(releaseDir, "catalogo.json"), manifestPath = path.join(releaseDir, "manifest.json"), releaseMetaPath = path.join(releaseDir, "release-meta.json"), buildInfoPath = path.join(releaseDir, "build-info.json"), materialsDir = path.join(releaseDir, "materials");
...
const catalog = readJSON(catalogPath), releaseManifest = readJSON(manifestPath), releaseMeta = readJSON(releaseMetaPath), buildInfo = readJSON(buildInfoPath), source = snapshotManifest.source || {};
const baselineQuestions = Number(source.previous_public_questions), expectedQuestions = Number(source.expected_public_questions), expectedMaterials = Number(source.expected_materials), expectedProofs = Number(source.expected_proofs), masterTotal = Number(source.master_total), discursive = Number(source.discursive_display_items || 0), expectedAwaiting = Number(source.expected_awaiting_audit || 0);
...
if (startingMaterials !== expectedMaterials) throw new Error(`O residual D não cria material: base ${startingMaterials}/${expectedMaterials}.`);
const materialId = materialIdByName.get(key(records[0].material_name));
if (!materialId || !materials.has(materialId)) throw new Error(`Material CRA-BA existente não encontrado: ${records[0].material_name}.`);
...
const publicQuestion = record => ({
  ...
  bloco: clean(record.block), disciplina: clean(record.discipline), assunto: clean(record.subject), subassunto: clean(record.subsubject), texto_base: clean(record.text_base), enunciado: clean(record.prompt),
  alternativas: {Certo: "Certo", Errado: "Errado"}, gabarito: record.answer, comentario: clean(record.comment), comentarios_alternativas: record.alternative_comments || {},
  fundamento: clean(record.foundation), pegadinha: clean(record.trap), observacoes: clean(record.observations), formato_questao: "Certo / Errado", pagina_pdf: clean(record.pdf_page),
  fonte_oficial: record.source_url, fonte_consolidada: record.source_url, auditoria: `Banco Mestre — ${operation} — transcrição, gabarito e conteúdo auditados`, notion_url: record.notion_url, codigo_fonte: record.code,
...
if (questionById.size !== expectedQuestions) throw new Error(`Lote residual D não fecha o catálogo: ${questionById.size}/${expectedQuestions}.`);
if (startingQuestions === baselineQuestions && added !== 2) throw new Error(`Adição parcial residual D: ${added}/2.`);
if (materials.size !== expectedMaterials || materials.size !== startingMaterials) throw new Error("Residual D alterou a quantidade de materiais.");
...
catalog.source = {...(catalog.source || {}), name: source.name || catalog.source?.name, notion_url: source.database_url || catalog.source?.notion_url, criteria: `${clean(catalog.source?.criteria)} + ${records.length} questões C/E reais Quadrix auditadas do lote ${operation}.`};
...
if (catalog.summary.banco_mestre - catalog.summary.questoes - catalog.summary.discursivas_consulta !== catalog.summary.aguardando_auditoria) throw new Error("Banco Mestre não fecha após residual D.");
...
releaseManifest.generated_at = snapshotManifest.captured_at; releaseManifest.summary = {...(releaseManifest.summary || {}), ...catalog.summary}; releaseManifest.catalog_sha256 = sha256(catalogText);
...
const receipt = {schema_version: "1.0", operation_id: operation, source_snapshot: "data/notion/quadrix-residual-gaps-d-20260816/manifest.json", status: "success", added_questions: added, total_questions: questionById.size, total_materials: catalogMaterials.length, total_proofs: proofs, banco_mestre: masterTotal, discursivas_consulta: discursive, aguardando_auditoria: expectedAwaiting, codes: [...expectedCodes]};
fs.writeFileSync(path.join(releaseDir, "quadrix-residual-gaps-d-20260816-receipt.json"), `${JSON.stringify(receipt, null, 2)}\n`);
console.log(`✓ Residual gaps D materializado: +${added} C/E no material CRA-BA existente; ${questionById.size} questões, ${catalogMaterials.length} materiais, ${proofs} provas; Banco Mestre ${masterTotal}.`);
