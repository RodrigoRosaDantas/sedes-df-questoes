...
if (historical.length !== 30) fail(`Esperados 30 registros históricos; encontrados ${historical.length}.`);
...
if (JSON.stringify(historicalCodes) !== JSON.stringify(expectedHistoricalCodes)) fail('Conjunto histórico de Contador deve corresponder exatamente aos códigos 071–100.');
...
for (const record of historical) {
  ...
  if (!expectedHistoricalNumbers.includes(Number(record.original_number))) fail(`${record.code}: número histórico fora do intervalo 71–100.`);
}
...
console.log('✓ Operação restrita validada: 70 questões novas de Contador, 30 históricas preservadas, sequência 001–070 e gates editoriais completos.');
