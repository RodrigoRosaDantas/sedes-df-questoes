...
const snapshotPath = 'data/notion/publication-additions/seedf-biomed-50.json';
const planPath = 'data/notion/publication-additions/seedf-biomed-50-plan.json';
const catalogPath = 'data/release/catalogo.json';
const manifestPath = 'data/release/manifest.json';
const receiptPath = 'data/release/seedf-biomed-50-publication-receipt.json';
...
for (const required of [snapshotPath, planPath, catalogPath, manifestPath]) if (!fs.existsSync(resolve(required))) fail(`Arquivo obrigatório ausente: ${required}.`);
const snapshot = readJSON(snapshotPath);
const plan = readJSON(planPath);
const catalog = readJSON(catalogPath);
const manifest = readJSON(manifestPath);

if (plan.total_records !== 50 || !Array.isArray(plan.lots) || plan.lots.length !== 1) fail('Plano de Biomedicina deve conter exatamente 50 registros em um lote.');
const [lot] = plan.lots;
if (lot.lot !== operationId || lot.expected_count !== 50 || JSON.stringify(lot.codes) !== JSON.stringify(expectedCodes)) fail('Plano de Biomedicina divergente.');
...
if (additions.some(record => !record)) fail('Snapshot de Biomedicina incompleto.');
for (const record of additions) {
  if (clean(record.github_id) || record.released_for_export !== true || record.publication_lot !== operationId) fail(`${record.code}: rastreabilidade, liberação ou lote inválido.`);
...
if (baseline !== 2609) fail(`Base pública antes de Biomedicina: ${baseline}; esperado 2609.`);
...
if (!target) {
  ...
  const material = {
    ...
    cargo: first.cargo || 'Professor de Educação Básica — Área de Formação: Biomedicina',
    codigo_cargo: first.cargo_code || '104',
    disciplina: first.discipline || 'Biomedicina',
...
if (catalog.summary.questoes !== 2659) fail(`Acervo após Biomedicina: ${catalog.summary.questoes}; esperado 2659.`);
...
const receipt = {
  schema_version: '1.0', operation_id: operationId, applied_at: new Date().toISOString(), baseline_questions: baseline,
  added_questions: 50, final_questions: catalog.summary.questoes, material_id: target.material.id,
  material_questions: target.material.quantidade_questoes, codes: expectedCodes,
  codes_sha256: sha256(`${expectedCodes.join('\n')}\n`), preserved_existing_catalog: true,
  source_snapshot: snapshotPath, source_plan: planPath,
};
...
if (Number(distCatalog.summary?.questoes) !== 2659) fail('Pacote público não contém 2659 questões.');
...
for (const metadata of distCatalog.materials || []) {
  ...
  for (const question of material.questoes || []) if (expectedSet.has(question.codigo)) occurrences.set(question.codigo, occurrences.get(question.codigo) + 1);
}
for (const code of expectedCodes) if (occurrences.get(code) !== 1) fail(`${code}: ocorrência pública ${occurrences.get(code)}.`);
console.log(`✓ Lote ${operationId} aplicado: 50 novas questões e 2659 no pacote público.`);
