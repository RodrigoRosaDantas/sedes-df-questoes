...
const manifestPath = path.join(root, "data", "notion", "quadrix-density-mapping-20260816", "manifest.json");
...
if (manifest.schema_version !== "1.0" || manifest.operation_id !== "SEDES-QDX-DENSITY-MAPPING-20260816") throw new Error("Manifesto de densidade ausente ou incompatível.");
if (!Array.isArray(manifest.mappings) || manifest.mappings.length !== Number(manifest.mapping_count) || Number(manifest.mapping_count) !== 14) throw new Error("Escopo do lote de densidade divergente.");
if (new Set(manifest.mappings.map(item => item.question_id)).size !== Number(manifest.distinct_questions) || Number(manifest.distinct_questions) !== 11) throw new Error("Quantidade de questões distintas do lote de densidade divergente.");
...
if (Number(catalog.summary?.banco_mestre) !== Number(totals.banco_mestre) || Number(catalog.summary?.questoes) !== Number(totals.public_questions) || Number(catalog.summary?.materiais) !== Number(totals.materials) || Number(catalog.summary?.provas) !== Number(totals.proofs) || Number(catalog.summary?.simulados) !== Number(totals.simulations) || Number(catalog.summary?.discursivas_consulta) !== Number(totals.discursive_display_items) || Number(catalog.summary?.aguardando_auditoria) !== Number(totals.awaiting_audit)) throw new Error("Catálogo mudou; o lote de densidade exige reauditoria editorial.");
...
for (const mapping of manifest.mappings) {
  ...
  const promptHash = sha256(clean(record.q.enunciado));
  if (promptHash !== mapping.prompt_sha256) throw new Error(`${mapping.question_id}: fingerprint do enunciado diverge do manifesto.`);
  if (formats[mapping.question_id] !== "Certo/Errado") throw new Error(`${mapping.question_id}: formato esperado é Certo/Errado.`);
  ...
  item.ce_question_ids = unique([...(item.ce_question_ids || []), mapping.question_id]);
...
fs.writeFileSync(path.join(releaseDir, "quadrix-density-mapping-20260816-receipt.json"), `${JSON.stringify(receipt, null, 2)}\n`);
console.log(`✓ Density mapping: 0 novas questões; ${receipt.mapping_pairs} vínculos em ${receipt.distinct_questions} questões Quadrix; ${receipt.mapped_questions} mapeadas; Tipificação e SIAFEM preservados em 1.`);
