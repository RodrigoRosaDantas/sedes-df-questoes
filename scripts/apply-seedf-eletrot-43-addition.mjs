...
const snapshotPath = 'data/notion/publication-additions/seedf-eletrot-43.json';
const planPath = 'data/notion/publication-additions/seedf-eletrot-43-plan.json';
const catalogPath = 'data/release/catalogo.json';
const manifestPath = 'data/release/manifest.json';
const receiptPath = 'data/release/seedf-eletrot-43-publication-receipt.json';
...
if (plan.total_records !== 43 || !Array.isArray(plan.lots) || plan.lots.length !== 1) {
  fail('Plano de Eletrotécnica deve conter exatamente 43 registros em um lote.');
}
const [lot] = plan.lots;
if (lot.lot !== operationId || lot.expected_count !== 43 || JSON.stringify(lot.codes) !== JSON.stringify(expectedCodes)) {
  fail('Plano de Eletrotécnica divergente.');
}
...
if (additions.some(record => !record)) fail('Snapshot de Eletrotécnica incompleto.');
...
for (const number of excludedNumbers) {
  ...
  if (byCode.has(code)) fail(`${code}: item bloqueado entrou indevidamente no snapshot restrito.`);
}

const baseline = Number(catalog.summary?.questoes);
if (baseline !== 2659) fail(`Base pública antes de Eletrotécnica: ${baseline}; esperado 2659.`);
...
if (!target) {
  ...
  const material = {
    ...
    cargo: first.cargo || 'Professor de Educação Básica — Área de Formação: Eletrotécnica',
    codigo_cargo: first.cargo_code || '111',
    disciplina: first.discipline || 'Eletrotécnica',
...
if (catalog.summary.questoes !== 2702) fail(`Acervo após Eletrotécnica: ${catalog.summary.questoes}; esperado 2702.`);
...
if (Number(distCatalog.summary?.questoes) !== 2702) fail('Pacote público não contém 2702 questões.');
...
for (const metadata of distCatalog.materials || []) {
  ...
  for (const question of material.questoes || []) {
    if (expectedSet.has(question.codigo)) occurrences.set(question.codigo, occurrences.get(question.codigo) + 1);
  }
}
for (const code of expectedCodes) if (occurrences.get(code) !== 1) fail(`${code}: ocorrência pública ${occurrences.get(code)}.`);
for (const number of excludedNumbers) {
  const code = `${prefix}${String(number).padStart(3, '0')}`;
  if (existingCodes.has(key(code))) fail(`${code}: item bloqueado já estava no catálogo anterior.`);
}
console.log(`✓ Lote ${operationId} aplicado: 43 novas questões e 2702 no pacote público.`);
