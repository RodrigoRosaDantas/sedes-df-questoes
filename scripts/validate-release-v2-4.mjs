...
for (const feature of ["answeredQuestionIds", "presentedQuestionIds", "ensureCanStartNewAttempt", "pagehide", "visibilitychange", "questionIds: state.questions.map", "loadQuestionsByIds", "Exportar progresso", "Importar backup", "aria-live=\"polite\"", "aria-current=\"step\"", "focusMainHeading", "result-options"]) {
...
for (const assignment of ['{id: "rodrigo", name: "Rodrigo", roles: ["202", "400"]}', '{id: "amanda", name: "Amanda", roles: ["202", "403"]}', '{id: "andressa", name: "Andressa", roles: ["200", "405"]}']) {
  if (!app.includes(assignment)) fail(`Vínculo de perfil ausente: ${assignment}`);
}
for (const selector of [".sr-only", ".dialog-backdrop", ".result-option", ":focus-visible", "prefers-reduced-motion", ".option .letter"]) {
  if (!styles.includes(selector)) fail(`Estilo obrigatório ausente: ${selector}`);
}
...
console.log(`✓ Release ${config.release_version} validada: ${questions} questões publicadas, ${trueFalseQuestions} C/E ativas e suporte nativo preparado para 240 itens em auditoria.`);
