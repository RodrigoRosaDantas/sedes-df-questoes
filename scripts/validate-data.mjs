...
if (!Array.isArray(catalog.bundle_chunks) || !catalog.bundle_chunks.length) fail("Partes do bundle ausentes.");
...
const materialIds = new Set(), questionIds = new Set(), questionCodes = new Set();
let total = 0, provas = 0, simulados = 0;
for (const meta of catalog.materials) {
  ...
  if (meta.tipo_material === "prova") provas++; else if (meta.tipo_material === "simulado") simulados++; else fail(`Tipo inválido: ${meta.id}`);
  const material = bundle.materials.find(item => item.id === meta.id);
  if (!material) fail(`Material ausente: ${meta.id}`);
  ...
  for (const question of material.questoes) {
    required(question.id, `ID em ${meta.id}`); required(question.codigo, `Código de ${question.id}`);
    required(question.enunciado, `Enunciado de ${question.id}`); required(question.comentario, `Comentário de ${question.id}`);
    ...
    questionIds.add(question.id); questionCodes.add(question.codigo);
...
if (bundle.materials.length !== catalog.materials.length) fail("Materiais excedentes ou ausentes.");
if (catalog.summary.materiais !== catalog.materials.length || catalog.summary.questoes !== total) fail("Resumo divergente.");
if (catalog.summary.provas !== provas || catalog.summary.simulados !== simulados) fail("Resumo por tipo divergente.");
if (total !== 180 || catalog.materials.length !== 9) fail(`Esperados 9 materiais e 180 questões; encontrados ${catalog.materials.length} e ${total}.`);
