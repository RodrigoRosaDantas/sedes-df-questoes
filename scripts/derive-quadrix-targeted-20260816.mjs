...
const expectedQuestions = Number(targetedManifest.source?.expected_public_questions || 0);
const targetedCodes = targetedManifest.expected_codes || [];
const edasCodes = edasManifest.expected_codes || [];
const targetedIds = targetedCodes.map(slug);
const edasIds = edasCodes.map(slug);
...
for (const questionId of [...targetedIds, ...edasIds]) if (!questionIndex[questionId]) throw new Error(`Questão de overlay ausente do catálogo final: ${questionId}.`);
...
for (const material of materials.values()) {
  for (const question of material.questoes || []) {
    ...
    searchItems.push({
      ...
      search: normalizeForSearch([material.nome, discipline, subject, material.fonte, material.ano, question.texto_base, question.enunciado, alternativesText, question.comentario, question.fundamento, question.pegadinha].filter(Boolean).join(" ")),
    });
...
const studyOutput = {
  ...
  generated_at: targetedManifest.captured_at,
  summary: {disciplines: studyDisciplines.size, topics: [...studyDisciplines.values()].reduce((sum, item) => sum + item.topics.size, 0), questions: expectedQuestions},
  disciplines: [...studyDisciplines.values()].map(discipline => ({
    ...
    topics: [...discipline.topics.entries()].map(([name, ids]) => ({name, question_count: ids.length, question_ids: ids})).sort((a, b) => b.question_count - a.question_count || a.name.localeCompare(b.name, "pt-BR")),
...
editalMap.generated_at = targetedManifest.captured_at;
...
console.log(`✓ Derivação final concluída: ${expectedQuestions} questões; ${formatSummary["true-false"]} C/E + ${formatSummary["multiple-choice"]} múltipla escolha; EDAS e lote Quadrix mapeados; cargos 202/400 sem tópicos vazios.`);
