...
test("home pública fica limpa e distingue catálogo de progresso", async ({page}) => {
  ...
  const catalogTime = page.locator("[data-ux15-sync-time]").first();
  await expect(catalogTime).toContainText(/\d{2}\/\d{2}\/\d{4}/);
  await expect(catalogTime.locator("..")).toContainText("Catálogo oficial atualizado em");
  const catalogAge = page.locator("[data-ux15-sync-age]").first();
  await expect(catalogAge).toContainText(/atualizad/);
  await expect(catalogAge).toHaveClass(/catalog/);
