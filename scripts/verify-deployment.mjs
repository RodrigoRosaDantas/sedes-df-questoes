...
const sourceReleaseMetaPath = path.join(root, "data/release/release-meta.json");
const sourceReleaseMeta = fs.existsSync(sourceReleaseMetaPath) ? JSON.parse(fs.readFileSync(sourceReleaseMetaPath, "utf8")) : {};
...
const expectedCacheVersion = String(sourceReleaseMeta.cache_version || `sedes-questoes-v${versionToken}`).trim();
...
if (!expectedCacheVersion.startsWith(`sedes-questoes-v${versionToken}`)) throw new Error(`Cache de referência incompatível com a versão ${packageData.version}: ${expectedCacheVersion}.`);
