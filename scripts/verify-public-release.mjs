...
if (questions !== 3492 || materials !== 85 || bank !== 3494 || Number(release.proofs) !== 46 || Number(release.simulations) !== 39 || discursive !== 2 || awaiting !== 0) throw new Error(`Totais finais inesperados: banco ${bank}, questões ${questions}, materiais ${materials}, provas ${release.proofs}, simulados ${release.simulations}, discursivas ${discursive}, auditoria ${awaiting}.`);
if (Number(format.summary?.["true-false"]) !== 2560 || Number(format.summary?.["multiple-choice"]) !== 932) throw new Error("Distribuição final de formatos divergente do lote aprovado.");
...
console.log(`✓ Auditoria reproduzível concluída no commit ${requestedSha.slice(0, 8)}: ${bank} no Banco Mestre = ${questions} objetivas + ${discursive} discursivas + ${awaiting} em auditoria; ${materials} materiais; lotes Quadrix 22/22 + CRESS MG/DF 12/12; Firebase/Work e endurecimentos protegidos por SHA-256; fontes canônicas preservadas em ${frozenRelease.files} arquivos.`);
