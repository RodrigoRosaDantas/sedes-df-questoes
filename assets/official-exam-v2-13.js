...
function balancedSelect(map, itemIds, total, excluded = []) {
  ...
  const selected = [];
  const pools = shuffle(itemIds || []).map(id => ({id, values: shuffle(byId.get(id)?.ae_question_ids || [])}));
  let progressed = true;
  ...
  if (selected.length < total) {
    const fallbackInsideEdital = shuffle([...new Set((itemIds || []).flatMap(id => byId.get(id)?.ae_question_ids || []))]);
...
function buildExamSelection(map, targetCode) {
  ...
  const forcedMaria = shuffle(target.maria_da_penha_ae_question_ids || []).slice(0, mariaRequired);
...
function deficitText(target) {
  ...
  const parts = [];
  if (deficits.general) parts.push(`${deficits.general} geral(is) A–E`);
  if (deficits.specific) parts.push(`${deficits.specific} específica(s) A–E`);
  if (deficits.maria_da_penha) parts.push(`${deficits.maria_da_penha} de Maria da Penha`);
  return parts.length ? `Faltam ${parts.join(", ")}.` : "Acervo apto para montar a prova.";
}
...
async function startOfficialExam(targetCode) {
  ...
  if (!selection) return toast("O acervo não conseguiu fechar a matriz oficial sem repetir ou sair do edital.", "error");
  ...
  try {
    ...
    createCompatibleSession({
      ...
      source: `${map.source?.notice || "Edital nº 1/2026"} — simulação por matriz oficial do cargo ${targetCode}`,
      cargo: targetCode,
...
function injectResult() {
  ...
  card.innerHTML = `<div><p class="eyebrow">Pontuação conforme o edital</p><h2>${total} de 100 pontos</h2><p>${eliminated ? "Abaixo de pelo menos um mínimo eliminatório." : "Mínimos eliminatórios atingidos nesta simulação."}</p><p class="official-target-label">${esc(blueprint.targetLabel || "Cargo")} · Cargo ${esc(blueprint.targetCode || "")}</p></div><div class="official-result-grid"><span><small>Gerais</small><strong>${general}/20</strong><em>mínimo 10</em></span><span><small>Específicas</small><strong>${specific}/80</strong><em>mínimo 40</em></span><span><small>Situação</small><strong>${eliminated ? "Risco" : "Apto"}</strong><em>simulação</em></span></div><p class="muted">A nota discursiva não está incluída.</p>`;
