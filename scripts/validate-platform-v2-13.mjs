...
const index = read("index.html"), worker = read("service-worker.js"), builder = read("scripts/build-public.mjs"), css = read("assets/platform-v2-13.css"), resolverStabilityCss = read("assets/resolver-stability-v2-23.css");
...
const references = ["release-v2-13.js?v=1", "vault-v2-13.js?v=1", "report-v2-13.js?v=2", "official-exam-v2-13.js?v=1", "adaptive-review-v2-13.js?v=1"];
...
requireMarkers(modules.shared, [
  ...
  "isTransientTimerMutation", "[data-total-time], [data-question-time]", "mutations.every(isTransientTimerMutation)",
], "Módulo compartilhado");
requireMarkers(resolverStabilityCss, ["font-variant-numeric:tabular-nums", "min-inline-size:96px", "[data-total-time],[data-question-time]"], "Estabilidade da resolução");
...
requireMarkers(modules.report, ["Reportar problema nesta questão", "questionReports.v1", "Enviar relato para revisão", "O relato fica salvo no seu progresso"], "Reporte interno");
if (modules.report.includes("issues/new")) fail("Reporte voltou a depender de GitHub Issue.");
...
requireMarkers(read(".github/ISSUE_TEMPLATE/problema-em-questao.yml"), ["Problema em questão", "Possível erro de gabarito", "Release/commit"], "Template administrativo legado");
...
console.log(`✓ Plataforma 2.13 validada: ${questions} questões, ${materials} materiais, carga inicial compartilhada, observers blindados, metadados em Configurações, cofre local, reporte interno, prova real e revisão adaptativa.`);