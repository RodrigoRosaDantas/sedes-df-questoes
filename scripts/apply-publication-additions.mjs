...
const snapshotPath = 'data/notion/publication-additions/seedf-dir-50.json';
const planPath = 'data/notion/publication-additions/seedf-dir-50-plan.json';
const catalogPath = 'data/release/catalogo.json';
const manifestPath = 'data/release/manifest.json';
const receiptPath = 'data/release/seedf-dir-50-publication-receipt.json';
const operationId = 'SEEDF-2025-DIR-A-071-120-20260802';
const expectedPrefix = 'PROVA-QDX-SEEDF-2025-DIR-A-';
const expectedCodes = Array.from({length: 50}, (_, index) => `${expectedPrefix}${String(index + 71).padStart(3, '0')}`);
...
if (plan.total_records !== 50 || !Array.isArray(plan.lots) || plan.lots.length !== 1) {
  fail('O plano adicional deve conter exatamente 50 registros em um único lote.');
}
const [lot] = plan.lots;
if (lot.lot !== operationId || lot.expected_count !== 50 || JSON.stringify(lot.codes) !== JSON.stringify(expectedCodes)) {
  fail('O plano adicional não corresponde exatamente às questões 71 a 120 de Direito.');
}
...
if (missing.length) fail(`O snapshot adicional está incompleto: ${missing.join(', ')}.`);
...
if (baselineCount !== 2535) fail(`Base pública divergente: ${baselineCount}; esperado 2535.`);
...
if (!target) {
  ...
  const material = {
    ...
    cargo: first.cargo || 'Professor de Educação Básica — Área de Formação: Direito',
    codigo_cargo: first.cargo_code || '108',
    disciplina: first.discipline || 'Direito',
...
if (catalog.summary.questoes !== 2585) fail(`Acervo após adição: ${catalog.summary.questoes}; esperado 2585.`);
...
if (actualNewCodes.size !== 50 || expectedCodes.some(code => !actualNewCodes.has(code))) {
  fail('O catálogo final não contém exatamente os 50 códigos autorizados.');
}
...
if (Number(distCatalog.summary?.questoes) !== 2585) fail('O pacote público não contém 2585 questões.');
...
console.log(`✓ Lote ${operationId} aplicado após a validação integral da base: 50 novas questões, 2585 no pacote público e nenhum código anterior removido.`);
