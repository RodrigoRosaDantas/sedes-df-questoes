...
async function prepareLocalState(page, callback) {
  await page.goto("./#/inicio", {waitUntil: "domcontentloaded"});
...
test("seleção do Estudo de hoje fica salva por perfil", async ({page}) => {
  await prepareLocalState(page, () => localStorage.removeItem("sedes.questoes.rodrigo.homeStudyToday.v2"));
