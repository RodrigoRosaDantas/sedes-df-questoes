...
if (releaseDir !== expectedReleaseDir) throw new Error("O lote residual B só pode ser materializado no dist; data/release permanece imutável.");
...
const operation = "SEDES-QDX-RESIDUAL-GAPS-20260816-B";

const snapshotManifest = readJSON(path.join(snapshotDir, "manifest.json"));
if (snapshotManifest.schema_version !== "1.0" || snapshotManifest.operation_id !== operation) throw new Error("Manifesto residual B ausente ou incompatível.");
if (Number(snapshotManifest.expected_count) !== 2 || Number(snapshotManifest.expected_materials_added) !== 2 || !Array.isArray(snapshotManifest.files) || snapshotManifest.files.length !== 1) throw new Error("Manifesto residual B com escopo inesperado.");
...
if (records.length !== 2 || expectedCodes.size !== 2) throw new Error(`Snapshot residual B incompleto: ${records.length}/2.`);
...
const catalog = readJSON(catalogPath), releaseManifest = readJSON(manifestPath), releaseMeta = readJSON(releaseMetaPath), buildInfo = readJSON(buildInfoPath);
...
for (const meta of catalog.materials || []) {
  ...
  for (const q of material.questoes || []) {
    if (q.id) questionById.set(key(q.id), q);
    if (q.codigo) questionByCode.set(key(q.codigo), q);
  }
}
...
const baseMaterial = record => ({
  id: `notion-${slug(record.material_name)}`, tipo_material: "prova", fonte: record.source_board || "Instituto Quadrix",
  nome: record.material_name, ano: Number(record.year), orgao: record.organization, cargo: record.cargo, codigo_cargo: String(record.cargo_code),
  disciplina: record.discipline, bloco: record.block, status: "publicado", source_url: record.source_url, formato_questao: "Múltipla escolha A–E",
  lote_publicacao: operation, comentarios_status: "concluido", quantidade_questoes: 0, tempo_sugerido_minutos: 0, questoes: [],
});
const publicQuestion = record => ({
  id: slug(record.code), codigo: record.code, numero: Number(record.original_number), numero_original: Number(record.original_number),
  bloco: clean(record.block), disciplina: clean(record.discipline), assunto: clean(record.subject), subassunto: clean(record.subsubject),
  texto_base: clean(record.text_base), enunciado: clean(record.prompt),
  alternativas: Object.fromEntries(["A", "B", "C", "D", "E"].map(letter => [letter, clean(record.alternatives[letter])])),
  gabarito: record.answer, comentario: clean(record.comment), comentarios_alternativas: record.alternative_comments || {},
  fundamento: clean(record.foundation), pegadinha: clean(record.trap), observacoes: clean(record.observations),
  formato_questao: "Múltipla escolha A–E", pagina_pdf: clean(record.pdf_page), fonte_oficial: record.source_url, fonte_consolidada: record.source_url,
  auditoria: `Banco Mestre — ${operation} — transcrição, gabarito e conteúdo auditados`, notion_url: record.notion_url, codigo_fonte: record.code,
  anulada: false, possui_imagem: false, descricao_imagem: "", imagem: "",
});
...
if (questionById.size !== expectedQuestions) throw new Error(`Lote residual B não fecha o catálogo: ${questionById.size}/${expectedQuestions}.`);
if (startingQuestions === baselineQuestions && added !== 2) throw new Error(`Adição parcial residual B: ${added}/2.`);
...
for (const material of [...materials.values()].sort((a, b) => String(a.nome).localeCompare(String(b.nome), "pt-BR"))) {
  ...
  const disciplines = new Set(material.questoes.map(q => clean(q.disciplina)).filter(Boolean));
  const blocks = new Set(material.questoes.map(q => clean(q.bloco)).filter(Boolean));
  ...
  for (const q of material.questoes) {
    if (questionIndex[q.id]) throw new Error(`ID duplicado: ${q.id}.`);
    questionIndex[q.id] = material.id;
  }
...
catalog.source = {...(catalog.source || {}), name: source.name || catalog.source?.name, notion_url: source.database_url || catalog.source?.notion_url,
  criteria: `${clean(catalog.source?.criteria)} + ${records.length} questões A–E reais Quadrix auditadas do lote ${operation}.`};
catalog.summary = {...(catalog.summary || {}), banco_mestre: masterTotal, materiais: catalogMaterials.length, questoes: questionById.size,
  aguardando_auditoria: expectedAwaiting, provas: proofs, simulados: simulations, discursivas_consulta: discursive};
catalog.materials = catalogMaterials; catalog.question_index = questionIndex;
if (catalog.summary.banco_mestre - catalog.summary.questoes - catalog.summary.discursivas_consulta !== catalog.summary.aguardando_auditoria) throw new Error("Banco Mestre não fecha após residual B.");
const catalogText = `${JSON.stringify(catalog, null, 2)}\n`;
fs.writeFileSync(catalogPath, catalogText);

const priorManifestById = new Map((releaseManifest.materials || []).map(item => [item.id, item]));
releaseManifest.generated_at = snapshotManifest.captured_at;
releaseManifest.summary = {...(releaseManifest.summary || {}), ...catalog.summary};
releaseManifest.catalog_sha256 = sha256(catalogText);
releaseManifest.materials = catalogMaterials.map(meta => {
  const file = path.join(materialsDir, `${meta.id}.json`), content = fs.readFileSync(file);
  return {...(priorManifestById.get(meta.id) || {}), id: meta.id, file: meta.file, questions: meta.quantidade_questoes,
    bytes: content.length, sha256: sha256(content), display_items: Number(priorManifestById.get(meta.id)?.display_items || 0)};
});
fs.writeFileSync(manifestPath, `${JSON.stringify(releaseManifest, null, 2)}\n`);
Object.assign(releaseMeta, {exported_at: snapshotManifest.captured_at, questions: questionById.size, materials: catalogMaterials.length, proofs,
  simulations, banco_mestre: masterTotal, awaiting_audit: expectedAwaiting, discursive_display_items: discursive});
fs.writeFileSync(releaseMetaPath, `${JSON.stringify(releaseMeta, null, 2)}\n`);
Object.assign(buildInfo, {questions: questionById.size, materials: catalogMaterials.length, material_files: catalogMaterials.length});
...
const receipt = {schema_version: "1.0", operation_id: operation, source_snapshot: "data/notion/quadrix-residual-gaps-b-20260816/manifest.json",
  status: "success", added_questions: added, total_questions: questionById.size, total_materials: catalogMaterials.length,
  total_proofs: proofs, banco_mestre: masterTotal, discursivas_consulta: discursive, aguardando_auditoria: expectedAwaiting, codes: [...expectedCodes]};
fs.writeFileSync(path.join(releaseDir, "quadrix-residual-gaps-b-20260816-receipt.json"), `${JSON.stringify(receipt, null, 2)}\n`);
console.log(`✓ Residual gaps B materializado: +${added} A–E; ${questionById.size} questões, ${catalogMaterials.length} materiais, ${proofs} provas; Banco Mestre ${masterTotal}.`);
